# What Does a Sim-to-Real Correlation Support? Five One-Line Checks and a Twenty-One-Paper Audit

*Draft v1.0 · 2026-07-20*
...
real-world policy success rates. We survey the twenty-one papers reporting such a correlation and find
that the field's central number rests on almost no independent data. Twenty of twenty-one compute
their headline correlation over fewer than ten independent training units; four compute it over
checkpoints or finetunes of a single policy — one unit; five report any uncertainty on it. Three print
...
call, only counting against each paper's own setup description. Four of the twenty-one surveyed
...
number revealed (MMRV is the field's ranking-agreement metric, defined in §7):
...
| real2sim-eval T-block MMRV = 0.108 | not attainable at the checkpoint count its own appendix states; attainable at the count our extraction recovers (§7.1) |
...
| REALM V-VIEW *r* = 0.89, MMRV = 0.253 | the *r* reproduces from the panel's 14 plotted points to 0.0002; the MMRV cannot be produced from them — and the panel draws 14 of the 21 points its own design implies (§7.2) |
...
**Precedent.** Instrumentation work publishes when it re-adjudicates a named number and ships a tool;
§1.1 places this paper against that line and against the closest existing audit.
...
claim that the simulator can stand in for hardware at all. Different object, different failure mode,
same direction of travel. Two routes reaching the same place is stronger evidence than either alone.

**What neither line covers.** Both that audit and `rliable` analyze data the benchmarks publish. The
papers we examine publish no machine-readable results at all (§2), so the prior question, *can this
...
Cousins' *r* is stable to unit composition, yet its k-unit interval is wide: the Fisher-z spans
[−0.410, +0.998] because *k* = 4 genuinely supports little. That is worth printing precisely because
it is not what a reader would infer from *r* = 0.91.
...
Throughout, the **subject paper** is real2sim-eval (arXiv:2511.04665), the paper whose data we recover
and re-analyze most deeply. No paper we surveyed publishes machine-readable evaluation results. We checked paper text, project
sites, and repositories: SimplerEnv (1,126★) and PolaRiS (221★, both as of 2026-07-20) ship simulation environments and code
...
container), one has no scatter plot. Of the nine papers added to the survey after this pass (six found
during earlier drafting, three via the completeness search of §8.1), three (Cosmos-Surg-dVRK, DreamDojo,
...
twenty-one. REALM, one of the original eight, was also re-extracted point-by-point across all four of
...
lattice implied by the stated trial count.
...
bounds, which recovers the episode counts themselves — sim *n* = 200, real *n* = 20/27/16 per task —
from the figure geometry alone.)*
...
This is the evidence for §2's **leverage** check. Removing one independent unit at a time from
RoboWorld, one of the survey's largest designs (*k* = 8 policies, reported *p* < 0.001):
...
This matters because §7 argues the *ranking* claim is what a simulator is really sold on. The accurate
statement is therefore narrower than "the result is fragile": RoboWorld's Pearson estimate is
leverage-dominated, and its ranking claim is not. We report this because omitting it, in a paper
that elsewhere argues ranking is the central claim, would have been the one robustness check that
happened not to get run.
...
**A reported *p*-value does not surface this.** *p* < 0.001 certifies the correlation is non-zero. It
does not certify that it survives removing one of eight policies, and here it does not. Nor would a
...
Dropping a *generalization level* also moves *r* four times more than dropping a *policy*.
Which axis you resample is not a detail, and no surveyed paper states one.
...
with the drop-one leverage and, since §4's rank-correlation check shows Pearson leverage need not
affect ranking, the same
statistic on Spearman. Drop-one is over independent units where they exist; rows marked *pts* have no
...
Pearson, 0.900 on Spearman) — an appendix panel sitting beside a headline pick correlation that passes
cleanly, which is exactly the situation the parenthetical exists to make visible. And two of REALM's
...
rerun, at the bottom of the table, passes everywhere at both episode scales (§13b).

Three things follow, and we think they favor keeping the check.

1. **A dominant unit *is* the failure mode.** Whether it is an isolated point at the leverage ceiling
   (RoboWorld: hat value 0.978 of a maximum 1.0) or a whole training run whose removal moves the
   estimate (Cosmos), the check identifies in one line the case where published decimals mislead.
2. **It is cheap and it clears most datasets.** Fifteen of twenty-two pass. A diagnostic that fired
   everywhere would be useless; one that fires on a third and points at a real design property is a
   filter, not an alarm. The 0.10 threshold is not tuned: any cutoff between 0.09 and 0.12 selects
...
Papers vary in whether they correlate *all* evaluated checkpoints or one representative per policy.
The choice is rarely stated and never justified:
...
The theoretical question — which estimand is *right* — we decline to settle: the all-checkpoint *r* is
pseudo-replicated (≈20 points from 4 runs), but the best-checkpoint *r* has *n* = 4 and corresponds to
what a practitioner actually does, and the literature has not addressed the choice. The practical
recommendation does not require settling it, and follows from §2.2's rule. The two choices are
...
report both correlations** — here +0.90 and −0.31, a pair far more informative than either alone. For
...
§2's **bootstrap-support** and **permutation** checks rest on a counting argument. Resampling *k*
...
not report a bootstrap CI.** Fisher-z will often return an interval spanning most of [−1, 1]. That is
the correct output, not a failure of the method: it is what *k* = 4 supports, and printing it is more
useful than printing a bootstrap interval that looks tighter because it has only 35 places to land.

*(A percentile interval remains formally defined at any k; what fails is coverage. On the three
checkpoint tasks only 1–3 atoms lie below the 2.5% cutoff, and on Digital Cousins four of 256
...
from the pooled all-checkpoint values in §4.2, which is §2.2's point about units restated). Aggregation
...
*How uncalibrated is it? Measured, not assumed (`fz_coverage.py`, released): in a simulation mirroring
our geometry (4 clusters × 4 points, cluster effects on both axes, 10,000 replicates per condition),
the pooled-center bound covered the unit-level correlation in 97–100% of replicates across ρ ∈
{0, 0.5, 0.9, 0.95} — conservative rather than calibrated, with median width 0.95–1.9 on the [−1, 1]
scale. The naive pooled Fisher-z with n = 16 (df = 13), the interval these designs invite, covered the
unit-level correlation in only 59–80% of replicates, dropping to 4–20% when within-cluster noise
carried no signal: that is the false precision pooling non-independent points produces. The bound's
one failure mode is leverage — displacing one cluster by 3 sd cut its coverage to 87.5% in the worst
condition — which is exactly the regime where §4's leverage check fires, and why the parenthetical
pairs them.*
...
\* RoboWorld's *x*-axis is a leaderboard score, so MMRV's |Rᵢ−Rⱼ| is min-max rescaled — an analogue, not
...
MMRV can only take multiples of 1/(*N* · *n*_ep), so a published value must land on that lattice within
3-decimal rounding slack. An earlier draft claimed this test is "discriminating in every case
...
**One verdict survives.** real2sim-eval's T-block MMRV of 0.108 misses
by 0.00137 at the *N* = 12 its own appendix states, on a lattice where only 19.2% of arbitrary values
pass. At the *N* = 15 our extraction recovered, the miss is 0.00033, inside the slack. The arithmetic
favors our recovered checkpoint count over the paper's stated one, which is independent support for
§3's extraction on the task where we most needed it.
...
brute-forcing violation and weighting variants until one matched two figures at once — and our own
first three attempts, each checking a plausible pair of conventions, wrongly concluded the values were
unreproducible. **The field's ranking metric is being computed under at least two argument orders —
SIMPLER's definition weights the real-side gap, its most transparent adopter weights the simulated
side — and no paper states which, or releases the code that would settle it.**

One panel resists even brute force. REALM's V-VIEW panel prints
*r* = 0.89 and MMRV = 0.253; our extraction reproduces the *r* from the panel's plotted points to
0.0002 — but the panel plots only 14 of the 21 points its 7-task × 3-policy design implies (the
close-drawer task is undrawn for every policy), and no MMRV computable from those 14 points comes
near 0.253 under some fifty definitional variants across two independent attempts (ours: 0.117). The
printed *r* and the printed MMRV of the same panel appear to come from different point sets. Its other
three panels reproduce both statistics cleanly, which is what makes the fourth diagnosable.

Appendix B records the investigation: the wrong explanation we published first, the alternatives
...
Two of §2's five checks need only *k*, the number of independent units — recoverable from every
paper's own setup description. So they can be run on all 21 surveyed papers without recovering any
data:

| result | papers |
|---|---|
| **Cannot reach *p* = 0.05 by any permutation test** | **6 / 21** — REALM, WorldGym, Cosmos-Surg-dVRK (*k*=3, min *p* = 0.167), WEAVER, EmbodiedSplat, Mem-World (*k*=2, 0.500) |
| *(check is category-inapplicable)* | 4 / 21 — RoboSnap, SC3-Eval, DreamDojo, dWorldEval correlate checkpoints or per-task finetunes of a **single** policy or lineage, so there are no units to permute |
| **Cannot support a percentile bootstrap CI** | **16 / 21** — at *k* ≤ 4 the resampling distribution has ≤ 35 attainable values |
| **Fisher-z has df ≤ 2** | **18 / 21** |
| Clears all three comfortably | **3 / 21** — RoboWorld and Gemini/Veo (*k* = 8), PlayWorld (*k* = 18) |

**Six papers report a correlation for which no permutation test over units could reach *p* = 0.05**,
because the minimum attainable value, 1/*k*!, exceeds it at their sample size. That minimum is
one-sided; two-sided it is at least as large, so the verdict is conservative as stated.
...
§8.1's Rule 1. EmbodiedSplat and Mem-World, both at *k* = 2, entered with the search. And four papers
are category-inapplicable rather than failed: RoboSnap, SC3-Eval, DreamDojo, and dWorldEval correlate
checkpoints or per-task finetunes of a *single* policy or lineage, so there are no units to permute.
Faulting a paper for failing a test it could not have run, and did not claim to run, is a technicality;
...
stricter reading of REALM's units — π0 and π0-FAST share a lineage — gives *k* = 2, which raises its
floor to 0.5.) The
finding is not that anyone failed a test. It is that the field routinely computes correlations over
one to four independent units, which is worth stating plainly and does not need the permutation
framing to land.

This is the cheapest thing in the paper: no extraction, no code beyond `factorial` and `comb`, and it
applies to every paper in the field on the day it is published.

![Figure 3](figures/fig3_survey_k_dotplot.png)

**Figure 3.** Independent training units behind the headline correlation, all 21 surveyed papers
...
**These limits are not consequences of the data being unavailable.** §3 reports that no surveyed paper
publishes machine-readable results, and it would be natural to read this section as downstream of that.
It is not. The minimum attainable *p*-value under a permutation test over *k* units is 1/*k*!, and the
bootstrap has at most C(2*k*−1, *k*) attainable values; both are combinatorial facts about *k*, which
follows from every paper's own setup description. Publishing the underlying data would not add a 36th
attainable value at *k* = 4, and would not let *k* = 3 reach *p* = 0.05. No release schedule moves
these ceilings.
...
| **6/21 cannot reach *p* = 0.05** | **no — this needs more policies** |
| **16/21 cannot support a bootstrap CI** | **no** |

The first three are limits on *our* verification. The last two are limits on what the *original*
analyses can conclude, and they survive every objection to our methods.
...
against its paper's full text on 2026-07-20; `survey_table.py` (released) regenerates every count in
this section from this table, so no number in §8 is hand-copied.
...
figures only. The last three rows entered via the documented completeness search (§8.1, Scope).
...
the **benchmarking audit** (§1.1) is out of scope. Listed so the exclusions are auditable too.

| property | prevalence |
|---|---|
| Headline *r* over **fewer than 10 independent training units** | **20 / 21** |
| **No uncertainty reported on the correlation** | **16 / 21** |
| Checkpoint-selection rule unstated | **16 / 21** |

Only PlayWorld (18 policies) clears ten independent units. Three papers compute a correlation over
two units. Four papers correlate checkpoints or per-task finetunes of a single policy or lineage — one
independent unit, the survey's most extreme design fact, invisible in our earlier counts.
...
miscredited uncertainty entries (§8.2), and re-coded three unit counts; the completeness search then
grew the set from 18 to 21. The corrected prevalences are higher for sample size, lower for the
selection rule. The earlier numbers were wrong in both directions, which is why this table exists.
...
SC3-Eval, Cosmos-Surg-dVRK, Gemini/Veo, dWorldEval, DreamDojo). No paper states which it intends,
so we state ours and a reader who disagrees can recompute.
...
|---|---|---|
| real2sim-eval | 12–20 checkpoints, or 4 training runs per task? | **4 runs per task** — the recovered Figure 3 data contains all four policies on every task, each with 3–6 checkpoints (12 task-policy series in total; Figure 4 displays six of them as unrolled curves). The "16/15/12 checkpoints" sentence describes its *appendix replay experiment*; Figure 3's caption states no counts. |
...
set is manipulation- and world-model-dominated (20 of 21); one navigation paper (EmbodiedSplat)
qualifies and is included; task type enters no check. The original collection was not systematic — six
...
Coding-independent: 18–20 of 21 papers report their headline correlation from fewer than ten units; at
least four (REALM, WEAVER, Mem-World, and WorldGym under two of three codings) cannot reach *p* = 0.05
by any permutation; and both reporting-practice counts (16/21 no uncertainty, 16/21 no stated rule)
involve no unit-counting at all. Coding-dependent: under the permissive coding the bootstrap-support
count falls from 16 to 7 of 21, the number clearing all three procedures rises from 3 to 11, and the
*k* = 1 category empties — so the strongest quantitative versions of those claims rest on Rule 1, and
we flag them as such. The below-floor set is nearly coding-independent: REALM and Mem-World print
...
RoboWorld (*p* < 0.001), REALM (*p* < 0.001), Cosmos-Surg-dVRK (*p* < 0.001), Mem-World (*p* < 0.001),
and MolmoSpaces (bracketed CIs on both R and ρ, printed inside its figure). Their existence is the
strongest argument that §2 is practical: five groups already do some of the harder part, computing and
reporting uncertainty on a correlation at these sample sizes, so the remaining sixteen papers' silence
reflects convention rather than difficulty. Three of the five, however, print *p*-values their own unit
...
An earlier draft counted four, crediting real2sim-eval's Figure 9 band and a WorldGym p-value.
Source verification removed both (the band is per-point episode uncertainty, Rule 2; WorldGym
prints no significance statistic anywhere) and added REALM's genuine p < 0.001, previously missed;
hence three. The miscredits survived two review passes and fell only on checking the source, which is
this section's argument, made at our own expense. The completeness search later added Mem-World and
MolmoSpaces to the reporting side of the ledger.

What §4 adds is the missing line. RoboWorld reports *p* < 0.001 on one of the survey's largest
designs, and §4 shows that its correlation still moves far on removing one of eight policies. A
...
findings the first run would have missed. We ship a linter that requires each stated requirement to be
claimed in code and to appear in the results file; Appendix C gives the full account, including the
linter's own first version failing its retrospective test.
...
Everything ships beside this paper; `README.md` documents the methodology, the equations behind every
...
| **The five checks** | `correlation_audit.py` — `--demo` reproduces §2.1 |
| **Survey table + counts** | `survey_table.py` — regenerates every §8 count |
| **Figures 1–3 + generator** | `figures/make_figures.py` — prints its verification numbers |
| **Test suite** | `tests/` — 18 known-answer tests + byte-for-byte §2.1/`--demo` parity |
| Preregistered analysis + results | `measure_noise_floor.py`, `results.json` |
| Fisher-z coverage simulation (§6.1) | `fz_coverage.py` — seed-fixed, 10,000 replicates per condition |
| Extraction-error bound (§3.1) | `pixel_reextract.py` |
| Environment | `requirements.txt` — pinned numpy and scipy |
| Subject-paper data (52 + 52 points) | `data/real2sim-eval-data/` — Fig. 3 checkpoints and the 200-episode Fig. 9 eval |
| RoboWorld (4 panels, 32 points) | `data/roboworld-data/` |
| Digital Cousins (16 points) | `data/digital-cousins-data/` |
| REALM (4 panels, 77 points) | `data/realm-data/` |
| Cosmos-Surg-dVRK (2 panels, 48 points) | `data/cosmos-surg-dvrk-data/` |
| DreamDojo (6 points) | `data/dreamdojo-data/` |
| MolmoSpaces (4 source panels, 24 rows) | `data/molmospaces-data/` |
| RoboSnap (10 points + its own table, Appendix A) | `data/robosnap-data/` |
| Preregistration | `PREREG-noise-floor.md` — including its hash-lock break record and v1.4 amendment |
| **Preregistration linter** | `harness/prereg_lint.py` *(repository root)* |
...
- **Not a claim about every paper in the field.** Twenty-one papers surveyed (five more examined and
  excluded), eleven recovered from figures; original collection non-systematic, extended by a
  documented completeness search (§8.1) whose own recall is limited.
...
from the released deterministic scripts and data; the verification protocol — multiple independent
recomputation passes, source-verified survey codings, adversarial validation that overturned two of
our own published-in-draft claims (§7.2, Appendix A) — is part of the record rather than a private
process.
...
does. It costs nothing to produce and it is the difference between a number a reader can act on and one
they cannot. (One label matters: where points outnumber units, the Fisher-z entry is a reference
bound, not a calibrated interval — §6.1 measures its coverage — and its blind spot, leverage, is
exactly what the drop-one range beside it exposes. The pair is honest where neither element alone is.)
...
>    rule decided the unit?** Checkpoints of one run are not units. (Twenty of twenty-one published
>    answers are below ten; four are one.)
...
Nor is the ask specific to physics simulators. Nine of the twenty-one surveyed evaluators are world
...
**(a) Which estimand, in theory?** §5 gives the practical answer — state which claim is being made,
and report both correlations when both claims are — but the underlying question stands: all-checkpoint *r* is pseudo-replicated, best-checkpoint *r* has
...
r/√(rel_x·rel_y) returns 1.13 (toy) and 1.09 (T-block): correlations above 1, which is not a
measurement of attenuation but a falsification of the model that produced it. Independent binomial
noise on the two axes cannot coexist with these observed *r*. Either the sim and real episode outcomes
are positively correlated (plausible under replay evaluation, where both axes score the same
trajectories), or the plug-in reliabilities are too noisy at *N* = 15–17 to support the correction.
Only rope yields a coherent corrected value (0.90 → 0.95). So we do not claim the true correlations are
higher than published. The design point also now has a direct empirical test: the subject paper's
...
only more units would. What survives is the design point, now with sharper support: episode noise is at
most a second-order problem next to the number of independent units, and the correction pinned at its
ceiling says the observed correlations are already as high as the error budget permits. Four
independent units is the limit, and no number of extra episodes fixes that.

---

## Before submission

1. **Author contact.** Draft prepared, not yet sent; this is the first referee question. The follow-up plan:
   send the extracted Figure 9 dataset back as validation, and ask which MMRV argument-order
   convention produced Table I (§7.2).
2. **Related work.** Position against *Deep RL that Matters*, *Statistical Precipice*, and the
   benchmarking audit (arXiv:2606.04233) — see §1.1, now written.
3. **Venue.** Determines length and apparatus.
4. **Completeness search.** The survey history stays as stated in §8.1 — it was not systematic and
   will not be rewritten as such. Before submission, run one documented arXiv full-text search (stated
   query strings, date, screened → included counts, coded per §8.1's rules, diffed against
   `survey_table.py`) and report it as a completeness check. **Done 2026-07-20**: seven queries, 30
   hits, three papers added (§8.1, Scope); rerun once more immediately before submission to catch
   newer postings.
...
§7.2 shows the MMRV mismatch has a different cause than we assumed, but the principle stands regardless.

## Appendix B — The MMRV mismatch: the wrong explanation, then the right one

The investigation record for §7.2, kept because we published the wrong explanation before finding the
right one.
...
*All 28 identifiers below were resolved against arXiv on 2026-07-20 and their titles confirmed to match.*

### Subject paper

- **real2sim-eval** — *Real-to-Sim Robot Policy Evaluation with Gaussian Splatting Simulation of
  Soft-Body Interactions.* arXiv:**2511.04665** (v1 and v2; v2 revises the MMRV column of Table I —
  see §7.1). Code: `github.com/kywind/real2sim-eval` (MIT). Assets: HuggingFace `shashuo0104`.

### Papers whose data we recovered and analyzed

- **RoboWorld** — *RoboWorld: Fast and Reliable Neural Simulators for Generalist Robot Policy Evaluation.* arXiv:**2607.01060** (v3; a v4 has since appeared — re-verify before submission). §2.1, §4, §7.
