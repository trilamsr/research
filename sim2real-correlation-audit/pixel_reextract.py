#!/usr/bin/env python
"""Pixel-based re-extraction of RoboWorld Fig 9a (arXiv:2607.01060v3).
...
def main() -> int:
    ...
    if not Path(args.pdf).is_file():
        sys.exit(f"error: PDF not found: {args.pdf}\n"
                 f"Download https://arxiv.org/e-print/2607.01060 (v3), extract the archive,\n"
    ...
    cL, cR = cluster(frame_cols)[0], cluster(frame_cols)[-1]
    ...
    print(f"|dr| pixel-vs-vector: {abs(r_pix - r_vec):.5f}   pixel-vs-published: {abs(r_pix - 0.989):.5f}")
    print(f"max |dx|: {np.max(np.abs(pX - vX)):.3f} RoboArena units ({np.max(np.abs(pX - vX))/xr*100:.2f}% of x-range)")
    print(f"max |dy|: {np.max(np.abs(pY - vY)):.4f} Score units ({np.max(np.abs(pY - vY))/yr*100:.2f}% of y-range)")
