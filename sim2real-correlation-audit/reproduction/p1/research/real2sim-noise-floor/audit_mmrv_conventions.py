...
regenerating code in the package:
  ...
  (a) The subject paper's (real2sim-eval, arXiv:2511.04665v2) Table I MMRVs
      ...
      (research/corpus-reporting-audit/sources/source-real2sim-eval-fig3-checkpoints.csv) under EXACTLY ONE convention in
  ...
  (b) The same convention reproduces the 200-episode appendix figure
      (research/corpus-reporting-audit/sources/source-real2sim-eval-fig9-200ep.csv) EXACTLY, as the rational lattice
  ...
  (c) REALM's (arXiv:2512.19562) V-VIEW panel prints MMRV = 0.253, and no
      ...
      points the panel actually draws (research/corpus-reporting-audit/sources/source-realm.csv); SIMPLER's own
...
USAGE.
  python research/real2sim-noise-floor/audit_mmrv_conventions.py            # full report + verdicts; exit 0 iff all
                                        # three published claims regenerate
  python research/real2sim-noise-floor/audit_mmrv_conventions.py --grid     # additionally dump all 60 variants for
                                        # every dataset
...
# Recovered from two figures: same violation predicate as SIMPLER,
...
def load_fig3():
    ...
    for r in _rows("source-real2sim-eval-fig3-checkpoints.csv"):
        n = int(r["n_episodes"])
...
def load_fig9():
    ...
    for r in _rows("source-real2sim-eval-fig9-200ep.csv"):
...
def load_realm_panel(panel: str):
    ...
    return [(float(r["x_real"]), float(r["y_sim"]))
            for r in _rows("source-realm.csv") if r["panel"] == panel]
