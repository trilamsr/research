"""Known-answer tests for research/real2sim-noise-floor/audit_mmrv_conventions.py — the §7.2 convention-grid brute force.
...
from audit_mmrv_conventions import (  # noqa: E402
...
def test_strict_gt_xor_equals_leq_xor_exhaustively_including_one_sided_ties():
    """Complementing both order predicates leaves XOR unchanged.

    The {-1, 0, +1} grid exhausts the sign/tie cases, including a tie on
    exactly one axis. This locks the authors' strict-> description to the
    audit's <=-XOR spelling without relying on a floating-point example.
    """
    for d_real, d_sim in product((-1, 0, 1), repeat=2):
        strict_gt_xor = (d_real > 0) != (d_sim > 0)
        leq_xor = VIOLATIONS["leq-xor"](d_real, d_sim)
        assert strict_gt_xor == leq_xor

    assert VIOLATIONS["leq-xor"](0, 1)
    assert VIOLATIONS["leq-xor"](1, 0)
    assert not VIOLATIONS["leq-xor"](0, 0)
